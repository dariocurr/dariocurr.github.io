...
export function pdf(shrinked) {
	...
	shrinked
		? pdfMake
			.createPdf(getResume())
			.download("Dario Curreri's Resume.pdf")
		: pdfMake
			.createPdf(getCurriculumVitae())
			.download("Dario Curreri's Curriculum Vitae.pdf");
...
function createExperienceAndEducationListCurriculumPDF(section) {
	...
	section.querySelectorAll("article").forEach((article) => {
		...
		var date =
			descriptionDiv.nextElementSibling.querySelector("span").innerHTML;
		content.push({
			columns: [
				{
					text: clean(
						descriptionDiv.querySelector("h3").innerHTML
					).toUpperCase(),
					style: "h3",
				},
				{
...
function getCurriculumVitae() {
	...
	var icons = [];
	Array.from(about.querySelectorAll(".social-icon")).forEach((socialIcon) => {
	...
	content.push({
		text:
			clean(publications.querySelector("h2").innerHTML).toUpperCase() + "\n\n",
		style: "h2",
	});
	content.push([
		{
...
function getResume() {
	...
	for (var i = 1; i < 4; i += 2) {
		...
		section.querySelectorAll("article").forEach((article) => {
			var descriptionDiv = article.firstElementChild;
			var dates = descriptionDiv.nextElementSibling
				.querySelector("span")
				.innerHTML.split(" - ")
				.map((date) => clean(date));
				...
				columns: [
					{
						stack: [
							{
								text: clean(descriptionDiv.querySelector("h3").innerHTML)
									.toUpperCase()
									.replaceAll("/", "\n"),
								style: "h3",
							},
							...
							{
								text: clean(descriptionDiv.querySelector("span").innerHTML)
									.toUpperCase()
									.replaceAll("/", "\n"),
								style: "subheading",
							},
