...
export function md() {
  ...
  md +=
    ...
    "](" +
    createLink(img.getAttribute("src")) +
    ")\n<br>\n\n";
  md +=
    extractTextMD(about.querySelector("span")) +
    " · [Online Curriculum Vitae](https://dariocurr.github.io/)" +
    "\n<br>\n\n";
  md += clean(about.querySelector("p").innerHTML) + "\n<br>\n\n";
  md += Array.from(about.querySelectorAll(".social-icon"))
    .map((socialIcon) => createLinkMD(socialIcon))
    .join(" ");
  md += "\n<br>\n<br>\n<br>\n\n";
  ...
  for (var i = 1; i < 4; i += 2) {
    ...
    section.querySelectorAll("article").forEach((article) => {
      ...
      md +=
        "### " + clean(descriptionDiv.querySelector("h3").textContent) + "\n";
      md +=
        "#### " +
        clean(descriptionDiv.querySelector("span").textContent) +
        "\n";
      md += "##### " + clean(dateDiv.querySelector("span").textContent) + "\n";
      article
        .querySelectorAll("p")
        .forEach((p) => (md += extractTextMD(p) + "\n\n"));
      md = md.substring(0, md.length - 1);
      article
        .querySelectorAll("li")
        .forEach((li) => (md += "- " + extractTextMD(li) + "\n"));
      md += "<br>\n<br>\n\n";
    });
    md = md.substring(0, md.length - 1);
    md += "<br>\n\n";
...
function createLinkMD(element) {
  return (
    "[" +
    clean(element.innerHTML) +
    "](" +
    createLink(element.getAttribute("href")) +
    ")"
  );
}
